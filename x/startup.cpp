...
int main(int argc, char **argv)
{
	...
	change_console_color(224);//无关紧要，只是改变打印出来的字体颜色与背景颜色
	cout<<"\n程序作者：付清旭（哈尔滨工业大学）\n\n\n";
	change_console_color(31);
	cout << "选择你要使用的功能：\n0: 调试软件\n1：游戏仿真\n2：读取已有图片处理\n";
	change_console_color(15);
	int function_selection = -1;//
	cin >> function_selection;//如果没有输入，则function_selection默认-1
	...
	switch (function_selection)
	{
		case 0:
			...
			break;
		case 1:
			virual_env_game();//游戏仿真
			break;
		case 2:
			read_picture();//读取已有图片处理
			break;
		default:
			break;
	}
...
void on_Mouse(int event, int x, int y, int flags, void *ustc)//event鼠标事件代号，x,y鼠标坐标，flags拖拽和键盘操作的代号
{
	...
	if (event == CV_EVENT_MOUSEMOVE)//读取鼠标所在坐标，并在图像上该点处划圆 （未实现），左键按下（未实现）
	{
		...
		if (x >= 0 && x < img_target_local.cols&&y>=0 && y < img_target_local.rows)
		{
			...
			if (img_target_local.dims == 2) //当图像为二维图像时,行与列的数目和
			{
			...
			{
				Vec3b pix = img_target_local.at<Vec3b>(row, col);//at函数将坐标所指点的信息传给pix
				B = pix[0];
				G = pix[1];
				R = pix[2];
			...
			sprintf(tmp, "POINT:(%d, %d)->RGB:(%d,%d,%d)                    ", row, col, R, G, B);//将信息（坐标与颜色）全部转成字符串
