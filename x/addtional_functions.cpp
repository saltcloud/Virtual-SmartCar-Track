...
Mat hwnd2mat(HWND hwnd)
{
	...
	HDC hwindowDC, hwindowCompatibleDC;//显示设备的句柄
	int height, width, srcheight, srcwidth;//高与宽，屏幕的高与宽
	HBITMAP hbwindow;//位图句柄
	Mat src;//
	BITMAPINFOHEADER  bi;//位图信息头部
	hwindowDC = GetDC(hwnd);//获取显示  我的世界 界面的设备
	hwindowCompatibleDC = CreateCompatibleDC(hwindowDC);//该函数创建一个与指定设备兼容的内存设备上下文环境（DC）。通过GetDc()获取的HDC直接与相关设备沟通，而本函数创建的DC，则是与内存中的一个表面相关联。
	SetStretchBltMode(hwindowCompatibleDC, COLORONCOLOR); //设置指定设备环境中的位图拉伸模式。被压缩时，被掩盖的部分不显示
	RECT windowsize;//窗口大小（左上角，右下角）
	...
	//	return x;
	//}
	src.create(height, width, CV_8UC4);//创建8位无符号整数矩阵/ 4通道图像，大小为height*width；CV_8UC3是一个8位无符号整数矩阵/ 3通道图像
	// create a bitmap
	hbwindow = CreateCompatibleBitmap(hwindowDC, width, height);//从 我的世界 里面获取 图像
	bi.biSize = sizeof(BITMAPINFOHEADER);//位图头部信息大小,具体配置参考  位图文件所包含的要素
	bi.biWidth = width;
	bi.biHeight = -height;  //this is the line that makes it draw upside down or not
	bi.biPlanes = 1;
	bi.biBitCount = 32;//32位图像
